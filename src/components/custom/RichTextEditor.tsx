...
declare module '@tiptap/core' {
  interface Commands<ReturnType> {
    setIframe: (options: { src: string; [key: string]: any }) => ReturnType;
  }
}
...
import { Button, Separator, Input, Textarea } from '../ui';
...
const Iframe = Node.create({
  ...
  addAttributes() {
    return {
      ...
      style: {
        default: 'border: 0; border-radius: 8px;',
      },
    };
  },
  ...
  renderHTML({ HTMLAttributes }) {
    return ['div', { class: 'iframe-wrapper my-4' }, ['iframe', mergeAttributes(HTMLAttributes)]];
  },
...
export interface RichTextEditorProps {
  ...
  readingMode?: boolean;
  extraSlashMenuOptions?: SlashMenuOption[];
}
...
export const RichTextEditor = forwardRef<RichTextEditorRef, RichTextEditorProps>(
  (
    ...
    const [isIframeDialogOpen, setIsIframeDialogOpen] = React.useState(false);
    const [iframeInput, setIframeInput] = React.useState('');
    ...
    const insertIframe = (code: string) => {
      const trimmed = code.trim();
      if (trimmed.startsWith('<iframe')) {
        editor.chain().focus().insertContent(trimmed).run();
      } else {
        (editor.chain() as any).focus().setIframe({ src: trimmed }).run();
      }
    };

    const setIframe = () => {
      if (iframeInput) {
        insertIframe(iframeInput);
      }
        ...
        >
          ...
          {!readingMode && (
            <BubbleMenu
              editor={editor}
              shouldShow={({ state }) => {
                if (isLinkEditorOpen) return true;
          ...
          {enableSlashMenu && !readingMode && (
            ...
            >
              ...
              {extraSlashMenuOptions?.map((option, index) => (
                <SlashMenuItem
                  key={index}
                  onClick={() => {
                    editor
          ...
          <DialogContent>
            ...
            <div className="py-4 space-y-2">
              ...
              <p className="text-[10px] text-muted-foreground italic">
                Note: Some sites may prevent being embedded. YouTube and Vimeo links work best.
              </p>
            </div>
            <DialogFooter>
              <Button variant="ghost" onClick={() => setIsIframeDialogOpen(false)}>
                Cancel
              </Button>
              <Button onClick={setIframe}>Embed</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
...
const SlashMenuItem: React.FC<SlashMenuItemProps> = ({ onClick, icon, label, className }) => (
  <button
    onClick={(e) => {
      e.preventDefault();
      e.stopPropagation();
